...
Before any exclusions, there are `r nrow(dat)` observations and 
`r length(unique(dat$user_id))` individuals in the dataset.

After excluding with missing sleep variables:

```{r}
cc <- dat |> drop_na(all_of(sleep_vars))
print_n(cc)
```
...
```{r}
tab_sup1 <- bind_rows(filter(dat, !(user_id %in% cc2$user_id)) |> mutate(samp = "Excluded"),
  ...
  tbl_summary(by = samp) |>
  as_gt() |>
  tab_header(title = md("**Supplementary Table 1:** Compared excluded vs. included individuals")) 
...
gtsave(tab_1, here("sleep", "writing", "tables", "tab_1.rtf"))
tab_1
